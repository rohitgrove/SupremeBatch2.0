import java.util.Scanner;

public class ConstructBinarySearchTree {
    public static Node insertIntoBST(Node root, int data) {
        if (root == null) {
            root = new Node(data);
            return root;
        }

        // it's not the first node
        if (data > root.data) {
            root.right = insertIntoBST(root.right, data);
        } else {
            root.left = insertIntoBST(root.left, data);
        }
        return root;
    }

    public static Node createBST() {
        Node root = null;
        Scanner sc = new Scanner(System.in);
        System.out.println("Enter data:");
        int data = sc.nextInt();

        while (data != -1) {
            root = insertIntoBST(root, data);
            System.out.println("Enter data:");
            data = sc.nextInt();
        }
        sc.close();
        return root;
    }

    public static Node createBST(int data[]) {
        Node root = null;
        for (int i = 0; i < data.length; i++) {
            root = insertIntoBST(root, data[i]);
        }

        return root;
    }

    public static void main(String[] args) {
        int data[] = { 50, 30, 60, 25, 40, 70, 80, 55 };
        Node root = createBST(data);
        Traversals.levelOrderTraversal(root);
        System.out.printf("InOrder: ");
        Traversals.inOrder(root);
        System.out.printf("\nPreOrder: ");
        Traversals.preOrder(root);
        System.out.printf("\nPostOrder: ");
        Traversals.postOrder(root);

        System.out.println();
        int minValue = MinimumAndMaximumValueInBST.minValue(root);
        if (minValue == -1) {
            System.out.println("There is no node in Tree, So no min value");
        } else {
            System.out.println("Min value: " + minValue);
        }

        int maxValue = MinimumAndMaximumValueInBST.maxValue(root);
        if (maxValue == -1) {
            System.out.println("There is no node in Tree, So no max value");
        } else {
            System.out.println("Max value: " + maxValue);
        }

        int t = 45;
        boolean ans = SearchInBST.searchInBST(root, t);
        if (ans) {
            System.out.println("Element is present");
        } else {
            System.out.println("Element is not present");
        }

        int key = 100;
        root = DeleteANodeFromBST.deleteNode(root, key);
        System.out.printf("\nPrinting Level Order Traversal: \n");
        Traversals.levelOrderTraversal(root);
    }
}
