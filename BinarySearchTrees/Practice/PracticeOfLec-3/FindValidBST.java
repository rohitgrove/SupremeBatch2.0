public class FindValidBST {
    static class NodeData {
        ...
        public NodeData(int size, int minValue, int maxValue, boolean isValidBST) {
            this.size = size;
    ...
    public static NodeData findLargestBST(Node root, int[] ans) {
        ...
        currNodeKaAns.minValue = Math.min(root.data, leftKaAns.minValue);
        currNodeKaAns.maxValue = Math.max(root.data, rightKaAns.maxValue);

        if (leftKaAns.isValidBST && rightKaAns.isValidBST
                && (root.data > leftKaAns.maxValue && root.data < rightKaAns.minValue)) {
