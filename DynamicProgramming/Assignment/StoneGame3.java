...
public class StoneGame3 {
    public static int solveUsingRecursion(int[] stoneValue, int idx) {
        ...
        for (int X = 1; X <= 3; X++) {
            ...
            ans = Math.max(ans, total - solveUsingRecursion(stoneValue, idx + X));
        }
    ...
    public static String stoneGameIII(int[] stoneValue) {
        // int ans = solveUsingRecursion(stoneValue, 0);
        // int dp[] = new int[stoneValue.length + 1];
        // Arrays.fill(dp, -1);
        // int ans = solveUsingMemo(stoneValue, 0, dp);
        int ans = solveUsingTabulation(stoneValue);
