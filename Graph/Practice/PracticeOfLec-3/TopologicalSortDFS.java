...
public class TopologicalSortDFS {
    ...
    public static void topologicalSortUsingDFS(int src, HashMap<Integer, Boolean> vis, Stack<Integer> st) {
        vis.put(src, true);

        if (adjList.containsKey(src)) {
            for (int nbr : adjList.get(src)) {
                if (!vis.containsKey(nbr)) {
                    topologicalSortUsingDFS(nbr, vis, st);
                }
            }
        }
    ...
    public static void topologicalSort(int n) {
        ...
        for (int i = 0; i < n; i++) {
            if (!vis.containsKey(i)) {
                topologicalSortUsingDFS(i, vis, st);
            }
        }
