import java.util.PriorityQueue;
import java.util.Comparator;
import java.util.ArrayList;
import java.util.List;
import java.util.Arrays;

class Pair {
    int element;
    int index;

    public Pair(int element, int index) {
        this.element = element;
        this.index = index;
    }
}

class CoustomComparator implements Comparator<Pair> {
    @Override
    public int compare(Pair o1, Pair o2) {
        return o2.element - o1.element;
    }
}

public class SlidingWindowMax {
    public static int[] maxSlidingWindow(int[] nums, int k) {
        PriorityQueue<Pair> pq = new PriorityQueue<>(new CoustomComparator());
        List<Integer> ans = new ArrayList<>();

        // consider first window
        for (int i = 0; i < k; i++) {
            pq.add(new Pair(nums[i], i));
        }
        // store max of first window
        ans.add(pq.peek().element);

        // consider rest of the windows.
        for (int i = k; i < nums.length; i++) {
            pq.add(new Pair(nums[i], i));

            // remove if maxs are from preious windows
            while (!pq.isEmpty() && pq.peek().index <= i - k) {
                pq.poll();
            }

            ans.add(pq.peek().element);
        }

        int res[] = new int[ans.size()];
        Arrays.setAll(res, ans::get);
        return res;
    }

    public static void printArr(int arr[]) {
        for (int i = 0; i < arr.length; i++) {
            System.out.print(arr[i] + " ");
        }
        System.out.println();
    }

    public static void main(String[] args) {
        int nums1[] = { 1, 3, -1, -3, 5, 3, 6, 7 }, k1 = 3;
        printArr(maxSlidingWindow(nums1, k1));
        int nums2[] = { 1 }, k2 = 1;
        printArr(maxSlidingWindow(nums2, k2));
    }
}
